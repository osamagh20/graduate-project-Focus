...
//    }
    ...
    public void CancelBooking(Integer id) {
        ...
        Shift shift = booking.getShift();
        //هنا لو كان نهاري او ليلي وكنسل,
        if (shift != null) {
            shift.setStatus("Available");
            if(! shift.getName().equalsIgnoreCase("Full Day")){
                Shift fullday=shiftRepository.findShiftBySpaceIdAndDateAndName(booking.getSpace().getId(),booking.getShift().getDate(),"Full Day");
                fullday.setStatus("Available");
